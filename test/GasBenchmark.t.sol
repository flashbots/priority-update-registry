...
contract GasBenchmarkTest is Test {
    ...
    function _signUpdate(address t, uint256 ts, uint256 cid, uint256[] memory slots)
        internal
        view
        returns (bytes memory)
    {
        bytes32 structHash =
            keccak256(abi.encode(registry.UPDATE_TYPEHASH(), t, ts, cid, keccak256(abi.encodePacked(slots))));
    ...
    function test_calculate_A0() public {
        ...
        r.updateState(t, block.timestamp, slots);
    ...
    function test_calculate_A1() public {
        ...
        r.updateState(t, block.timestamp, slots0);
        ...
        r.updateState(t, block.timestamp, slots1);
    ...
    function _prepareBatch(uint256 n, uint256 k) internal returns (PrioUpdateRegistry.SignedUpdate[] memory updates) {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            bytes memory sig = _signUpdate(t, block.timestamp, block.chainid, slots);
            updates[i] = PrioUpdateRegistry.SignedUpdate(t, block.timestamp, block.chainid, slots, sig);
        }
    }
