...
- **`updateStateWithDecoder(address target, uint256 laneIndex, bytes aux)`**
  Permissionless relay for a decoder-managed lane. The registered decoder validates `aux` and returns the slots to store. The registry calls the decoder with `STATICCALL` and performs the storage writes itself.
...
Anyone can relay `updateStateWithDecoder`. The decoder must authenticate and validate the payload completely. A decoder that accepts arbitrary input gives arbitrary callers control over its lane. Decoder addresses are permanent, but proxy decoders may remain upgradeable.
