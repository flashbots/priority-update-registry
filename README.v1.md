...
`ExamplePropAmm` is a minimal proprietary AMM that reads its per-pair pricing parameters (`concentration`, `multX`, `multY`) from the V1 registry. The market maker publishes a priority update each block. Swappers read the latest parameters via `getState`, with the registry enforcing a `maxParameterAge` freshness window. Adapted from [fahimahmedx/prop-amm](https://github.com/fahimahmedx/prop-amm), which uses a different top-of-block storage mechanism.
